...
def send_command(action: str, timeout: float = 180.0, **kwargs) -> dict:
    ...
    def ensure_current_life(evolution_state: dict, state: dict) -> LifeTelemetry:
        ...
        next_number = int(evolution_state.get("next_life_number", evolution_state.get("life_number", 1)) or 1)
        ...
        # life_id is always a UUID independent of life_number.  An aborted attempt
        # keeps the same life_number but receives a fresh life_id next time.
    ...
    def record_life_terminal(evolution_state: dict, life: LifeTelemetry,
        ...
        if terminal_state == "dead":
            ...
            # AUTHORITATIVE duplicate-death guard.  The consumed-status file is only
            # a diagnostic cache and is never used to decide whether a death counted.
        ...
        # Any terminal record closes this attempt.  If it was aborted, the next
        # attempt keeps the same life_number but gets a new UUID.
    ...
    def recover_previous_runtime(evolution_state: dict, log_path: Path) -> bool:
        ...
        if not status or str(status.get("status", "")) != "dead":
            # Stale active-life rule is resolved by a real observe in main:
            # alive observe -> resume; dead observe/status -> close as dead;
            # observe failure -> close marker as aborted.
            return False
        ...
        if marker and int(marker.get("schema_version", -1)) == LIFE_RECORD_SCHEMA_VERSION:
            ...
            life = LifeTelemetry(
                ...
                life_number=int(evolution_state.get("next_life_number", evolution_state.get("life_number", 1)) or 1),
        ...
        # Cache/rotation happens only AFTER evolution state has durably recorded the
        # death signature. Evolution state is authoritative.
        archive_consumed_life_status(status)
        return True
    ...
    def log_life_started(log_path: Path, life: LifeTelemetry, resumed: bool) -> None:
        ...
        append_log(log_path, {
            ...
            # Runtime/session context only. This list is NOT included in Qwen prompts.
